...
func TestBug02NamespaceScopedDigest(t *testing.T) {
	...
	if err != nil {
		t.Errorf("different namespace must not conflict: %v", err)
	}
	if err == nil && (b.RequestID == a.RequestID || !b.Created) {
		t.Errorf("namespace collision merged requests: a=%#v b=%#v", a, b)
	}
	...
	if !errors.Is(err, application.ErrConflict) {
		t.Errorf("same namespace accepted a conflicting digest: %v", err)
	}
	...
	if err != nil || replayed.Created || replayed.RequestID != b.RequestID {
		t.Errorf("legal idempotent retry failed after scoped conflict: response=%#v err=%v", replayed, err)
	}
	page, err := s.QueryRequests(ctx, application.QueryRequest{CallerID: "robot"})
	if err != nil || page.Total != 2 || len(page.Items) != 2 {
		t.Errorf("scoped conflict changed existing requests: page=%#v err=%v", page, err)
	}
	...
	if batch.Failed != 1 || batch.Succeeded != 0 {
		t.Errorf("partial result counters are inconsistent: %#v", batch)
	}
}
